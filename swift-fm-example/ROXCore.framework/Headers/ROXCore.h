...
/**
 Multi-SDK support: Switch device properties context for the given SDK key
 Internal method used by multi-configuration system to ensure correct BUID isolation
 */
+(void)switchToSDKKey:(NSString * _Nonnull)sdkKey;

/**
 Multi-SDK support: Get the current SDK key
 @return The current SDK key being used, or nil if not set
 */
+(NSString * _Nullable)currentSDKKey;

/**
 Multi-SDK support: Get the flags repository for a specific SDK key
 @param sdkKey The SDK key to get the flags repository for
 @return The flags repository for the specified SDK key, or the global repository if not found
 */
+(ROXFeatureFlagsRepository* _Nonnull)flagsRepositoryForSDKKey:(NSString* _Nonnull)sdkKey;
